...
public class OrganizerApplicationReviewController : ControllerBase
{
    ...
    [HttpPost("{applicationId:guid}/start-review")] public async Task<IActionResult> StartReview(Guid applicationId, CancellationToken ct) { var result = await _reviewService.StartReviewAsync(User.GetUserId(), applicationId, ct); return result.IsSuccess ? Ok() : BadRequest(new { Error = result.Error }); }
